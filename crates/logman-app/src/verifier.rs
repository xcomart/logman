//! Host key policy for logman.
//!
//! logman follows the *trust on first use* convention that OpenSSH popularised:
...
//!
//! The database lives in [`logman_core::KnownHosts`]; this module only supplies
//! the policy that sits between it and [`logman_ssh`].

use std::sync::{Arc, Mutex};

use logman_core::{HostKeyStatus, KnownHosts};
use logman_ssh::{HostKeyVerifier, algorithm_name, fingerprint};
use russh::keys::PublicKey;

/// Trust-on-first-use policy backed by [`logman_core::KnownHosts`].
///
...
/// [`SshEvent::HostKey`](logman_ssh::SshEvent::HostKey) with `accepted: false`,
/// immediately followed by
/// [`SshEvent::Error`](logman_ssh::SshEvent::Error) carrying
/// [`SshErrorKind::HostKeyRejected`](logman_ssh::SshErrorKind::HostKeyRejected).
