...
use logman_term::{Rgb, SchemeFile, TerminalTheme};
use serde::Serialize;
...
pub struct CatalogEntry {
    ...
    /// Human-readable name.
    pub name: String,
    /// Whether the entry ships with logman rather than coming from a file.
    pub builtin: bool,
}
...
/// file [`SchemeFile`] mirrors — with no wrapper of logman's own.
...
impl Catalog {
    ...
    pub fn directory(self) -> Result<PathBuf> {
        match self {
            Self::UiTheme => logman_core::ui_themes_dir(),
            Self::Scheme => logman_core::schemes_dir(),
        }
    }
...
/// because the languages logman ships disagree about where the word goes —
...
/// file logman saves says what it means rather than leaning on a derivation.
