...
//! outside the directory logman is already installed in, and nothing is
...
const LATEST_RELEASE_API: &str = "https://api.github.com/repos/xcomart/logman/releases/latest";
...
const RELEASES_PAGE: &str = "https://github.com/xcomart/logman/releases";
...
const FALLBACK_ARCHIVE: &str = "logman-update";
...
/// An Intel Mac or an ARM Linux box runs a locally built logman, and there is
...
#[cfg(windows)]
const PAYLOAD: &str = "logman.exe";
/// See the Windows variant above.
#[cfg(target_os = "macos")]
const PAYLOAD: &str = "logman.app";
/// See the Windows variant above.
#[cfg(all(unix, not(target_os = "macos")))]
const PAYLOAD: &str = "logman";

/// A downloadable build of a release, matched to this target triple.
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct Asset {
    /// File name as published, e.g. `logman-v0.4.0-x86_64-pc-windows-msvc.zip`.
    pub name: String,
...
/// own — logman is not quarantine-aware, and `tar` restores none from the
...
fn download(asset: &Asset, to: &Path, report: &mut dyn FnMut(Progress)) -> Result<(), String> {
    ...
    let mut response = agent
        .get(&asset.url)
        .header("User-Agent", format!("logman/{CURRENT_VERSION}"))
...
/// What this run of logman would replace: the executable, or on macOS the
...
fn install_target() -> Result<PathBuf, String> {
    ...
    {
        bundle_root(&exe)
            .ok_or_else(|| "logman is not running from an application bundle".to_string())
    }
...
///
/// `current_exe()` in a bundle is `<name>.app/Contents/MacOS/logman`, but the
...
/// `logman.exe` becomes `logman.exe.old` and not `logman.old`: the second would
...
fn asset_name(tag: &str, target: &str) -> String {
    ...
    format!("logman-{tag}-{target}.{extension}")
}
...
/// rejected rather than truncated — logman does not publish them, so a tag
...
mod tests {
    ...
    fn a_release_response_yields_its_tag_and_page() {
        ...
        let body = r#"{
            ...
            "name": "logman 0.4.0",
            "draft": false,
            "html_url": "https://github.com/xcomart/logman/releases/tag/v0.4.0",
            "assets": []
        }"#;
        ...
        assert_eq!(
            release.url,
            "https://github.com/xcomart/logman/releases/tag/v0.4.0"
        );
        assert_eq!(
            release_url(&release),
            "https://github.com/xcomart/logman/releases/tag/v0.4.0"
    ...
    fn an_asset_name_follows_the_release_workflow() {
        assert_eq!(
            ...
            "logman-v0.4.0-x86_64-pc-windows-msvc.zip"
        );
        assert_eq!(
            asset_name("v0.4.0", "aarch64-apple-darwin"),
            "logman-v0.4.0-aarch64-apple-darwin.tar.gz"
        );
        assert_eq!(
            asset_name("v0.4.0", "x86_64-unknown-linux-gnu"),
            "logman-v0.4.0-x86_64-unknown-linux-gnu.tar.gz"
        );
    }
    ...
    fn the_displaced_copy_keeps_its_whole_name() {
        for (path, expected) in [
            ("C:/Program Files/logman/logman.exe", "logman.exe.old"),
            ("/usr/local/bin/logman", "logman.old"),
            ("/Applications/logman.app", "logman.app.old"),
        ] {
    ...
    fn a_bundle_is_found_however_deep_the_binary_sits() {
        assert_eq!(
            bundle_root(Path::new("/Applications/logman.app/Contents/MacOS/logman")),
            Some(PathBuf::from("/Applications/logman.app"))
        );
        // The extension is what identifies it, not the depth or the name.
        assert_eq!(
            bundle_root(Path::new("/tmp/x/Some Name.APP/Contents/MacOS/logman")),
        ...
        assert_eq!(
            bundle_root(Path::new("/work/logman/target/debug/logman")),
            None
        );
        assert_eq!(bundle_root(Path::new("/usr/local/bin/logman")), None);
    }

    #[test]
    fn an_archive_name_can_never_leave_the_staging_directory() {
        assert_eq!(
            archive_name("logman-v0.4.0-x86_64-pc-windows-msvc.zip"),
            "logman-v0.4.0-x86_64-pc-windows-msvc.zip"
        );
    ...
    fn the_payload_is_found_at_the_root_or_one_level_down() {
        ...
        // Nothing there yet.
        assert_eq!(find_payload(root, "logman"), None);

        // The shape every published archive has: one wrapper directory.
        let wrapper = root.join("logman-v0.4.0-x86_64-unknown-linux-gnu");
        fs::create_dir_all(&wrapper).expect("a wrapper directory");
        fs::write(wrapper.join("logman"), b"binary").expect("a payload");
        assert_eq!(find_payload(root, "logman"), Some(wrapper.join("logman")));

        // A flat archive works too, and wins, because it is unambiguous.
        fs::write(root.join("logman"), b"binary").expect("a payload");
        assert_eq!(find_payload(root, "logman"), Some(root.join("logman")));

        // A directory counts as a payload: that is what the macOS bundle is.
        let bundles = tempfile::tempdir().expect("a temp directory");
        fs::create_dir_all(bundles.path().join("wrapper/logman.app/Contents")).expect("a bundle");
        assert_eq!(
            find_payload(bundles.path(), "logman.app"),
            Some(bundles.path().join("wrapper/logman.app"))
        );
    }
