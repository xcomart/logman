...
//!
//! Six themes ship with logman, one per built-in terminal color scheme and
...
pub struct ThemeEntry {
    ...
    /// Whether the palette is a dark one.
    pub dark: bool,
    /// Whether the theme ships with logman rather than coming from a file.
    pub builtin: bool,
}
...
/// terminal schemes, which `logman-term` has to resolve without an [`App`] in
...
impl ThemeRegistry {
    ...
    /// Whether `id` names a theme that ships with logman.
...
/// logman does not know are ignored, and a color it cannot parse falls back to
...
mod tests {
    ...
    fn the_builtin_themes_mirror_the_terminal_schemes() {
        ...
        let schemes = logman_term::TerminalTheme::builtin();
