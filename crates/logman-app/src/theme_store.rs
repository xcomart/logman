...
//!
//! logman ships six themes and six schemes; anything beyond that comes from a
//! `*.json` file dropped into [`logman_core::ui_themes_dir`] or
//! [`logman_core::schemes_dir`]. Each file's stem is the id the theme or scheme
//! is selected by, so `~/.config/logman/schemes/tokyo-night.json` is the scheme
//! `tokyo-night`.
//!
...
//!
//! The formats are [`crate::ui::ThemeFile`] and [`logman_term::SchemeFile`];
...
use logman_term::{CustomScheme, SchemeFile, TerminalTheme};
...
/// the time the first frame is drawn — and again after every change logman
...
/// Whether `id` names a scheme that ships with logman.
...
fn validated_id(id: &str, is_builtin: fn(&str) -> bool) -> Result<String> {
    ...
    if is_builtin(&slug) {
        bail!("{slug} is the id of a theme that ships with logman");
    }
    Ok(slug)
}
...
fn load_dir<T: DeserializeOwned>(
    ...
    for entry in entries.flatten() {
        ...
        if is_builtin(&id) {
            log::warn!(
                "skipping {}: {id} is the id of a {kind} that ships with logman",
                path.display()
            );
            continue;
        }
