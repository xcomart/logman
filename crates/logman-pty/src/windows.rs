...
//! — `logman-term`, which already replies to device status reports — is what
...
fn control(
    ...
    let spawned = std::thread::Builder::new()
        .name("logman-pty-reader".to_owned())
    ...
    let waiter = std::thread::Builder::new()
        .name("logman-pty-waiter".to_owned())
        .spawn(move || {
