...
fn publishes_ready_then_output_then_exited() {
    let (session, mut events) =
        PtySession::spawn(config_running(&["cmd.exe", "/c", "echo hello-logman"]));
    ...
    assert!(
        output(&seen).contains("hello-logman"),
...
fn writes_input_to_the_shell_and_ends_on_shutdown() {
    ...
    session.send_input(b"echo typed-logman\r\n".to_vec());

    let echoed = drain(Some(&session), &mut events, |seen| {
        output(seen).contains("typed-logman")
    });
    assert!(
        output(&echoed).contains("typed-logman"),
...
fn a_command_that_does_not_exist_is_reported_as_an_error() {
    let (session, mut events) =
        PtySession::spawn(config_running(&["logman-pty-test-shell-that-is-not-there"]));
